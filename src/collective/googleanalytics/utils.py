...
def evaluateTALES(parent, exp_context, evaluate_keys=False):
    ...
    if type(parent) is dict:
        ...
        for key, value in parent.items():
            if evaluate_keys:
                results[evaluateTALES(key, exp_context)] = \
                    evaluateTALES(value, exp_context)
            else:
...
def json_serialize(parent):
    ...
    if type(parent) is dict:
        for (key, value) in parent.items():
            json_parts.append('%s: %s' % (json_serialize(key),
...
def extract_value(column):
    ...
    if column.name == 'ga:date':
        ...
        value = int(value)
    elif column.type in ['float', 'percent', 'time', 'currency',
                         'us_currency']:
        value = float(value)

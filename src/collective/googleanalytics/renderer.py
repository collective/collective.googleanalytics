...
from collective.googleanalytics.visualization import \
    AnalyticsReportVisualization
...
def renderer_cache_key(method, instance):
    ...
    cache_vars = [time_key, modification_time, report_id,
                  tuple(instance.profile_ids())]
...
class AnalyticsReportRenderer(object):
    ...
    def data(self):
        ...
        for entry in self._getDataFeed().entry:
            results.append(dict([extract_value(row) for row in
                                entry.dimension + entry.metric]))
        return results
    ...
    def dimension(self, dimension, specified={}, aggregate=unique_list,
                  default=[]):
        """
    ...
    def possible_dates(self, dimensions=[], aggregate=list):
        ...
        DATE_DIMENSIONS = ['ga:date', 'ga:day', 'ga:week', 'ga:month',
                           'ga:year']
        ...
        for delta_days in range(0, delta.days + 1):
            ...
            if 'ga:week' in date_dimensions:
                ...
                values['ga:week'] = int(
                    math.ceil(float(ytd_days - firstweek_days)/7)) + 1
    ...
    def _getDataFeed(self):
        ...
        data_feed = analytics_tool.makeClientRequest(
            'data', 'GetData', **query_args)
    ...
    def _getVisualization(self):
        ...
        options = evaluateTALES(dict([v.split(' ', 1) for v in
                                self.report.viz_options]), exp_context)

        return AnalyticsReportVisualization(
            self.report, self.columns(), self.rows(), options)
    ...
    def _getValue(self, name, specified={}, aggregate=sum, default=0):
        ...
        values = [e[name] for e in self.data() if set(
            specified.items()) <= set(e.items())]
        if not values:
            return default

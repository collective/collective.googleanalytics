...
from collective.googleanalytics.interfaces import \
    GoogleAnalyticsMessageFactory as _
...
class IAnalyticsTracking(Interface):
    ...
    tracking_plugin_names = schema.List(
        ...
        value_type=schema.Choice(
            vocabulary='collective.googleanalytics.TrackingPluginNames'),
        default=[],
    ...
    tracking_excluded_roles = schema.List(
        ...
        value_type=schema.Choice(
            vocabulary='collective.googleanalytics.Roles'),
        default=[],
        description=_(
            u"Choose the roles that should be excluded from tracking."),
        required=False)
...
class IAnalyticsSettings(Interface):
    ...
    cache_interval = schema.Int(
        ...
        description=_(
            u"Enter the number of minutes for which account \
            information and report results should be cached."),
        default=60,
        required=True)

    date_range = schema.Choice(
        title=_(u"Date Range"),

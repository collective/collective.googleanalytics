...
class Analytics(PloneBaseTool, IFAwareObjectManager, OrderedFolder):
    ...
    tracking_excluded_roles = FieldProperty(IAnalytics[
    ...
    def _getAuthenticatedClient(self, service='data'):
        ...
        if not self.auth_token:
            raise error.BadAuthenticationError, \
                'You need to authorize with Google'
    ...
    def getClients(self):
        ...
        # We store the clients object on the ZODB database connection.
        # This approach comes from alm.solrindex. See the documentation
        # for that product for a full explanation.
        jar = self._p_jar
        oid = self._p_oid
    ...
    def makeClientRequest(self, service, method, *args, **kwargs):
        ...
        # Workaround for the lack of timeout handling in gdata. This approach
        # comes from collective.twitterportlet. See:
        ...
        try:
            ...
            try:
                ...
                if hasattr(e, 'reason'):
                    ...
                    reason = e[0]['reason']
                if 'Token invalid' in reason or reason in (
    ...
    def getAccountsFeed(self, feed_path):
        ...
        feed_url = 'https://www.googleapis.com/analytics/v2.4/management/' + \
            feed_path
        token = gdata.gauth.AuthSubToken(self.auth_token)
        client = gdata.analytics.client.AnalyticsClient(auth_token=token)
        res = self.makeClientRequest(
            None, client.get_management_feed, feed_url)
        return res

...
class TestReports(FunctionalTestCase):
    ...
    def test_default_expression_context(self):
        """
        Test that the default expression context used to render TAL and TALES
        in the Analytics report is populated with the correct objects.
        """
    ...
    def test_report_returns_correct_results(self):
        """
        Test that the report's getResults method produces an
        AnalyticsReportResults object with the correct values.
        """
        ...
        feed_xml_file = os.path.join(
            os.path.dirname(__file__), 'data_feed.xml')
        ...
        results_js_file = os.path.join(
            os.path.dirname(__file__), 'report_results.tpl')
